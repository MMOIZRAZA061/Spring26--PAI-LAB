...
# Optional dependencies for embeddings and FAISS
try:
    import faiss
except Exception:
    faiss = None
...
# Add a couple extra patterns for local language/common shorthand
pairs.extend([
    [r"(.*)address(.*)|(.*)batao(.*)|(.*)kahan(.*)|(.*)kaha(.*)", ["We are located in the main city center — call 0300-1234567 for directions."]],
    [r"more|tell me more|details(.*)", ["Which item would you like more details about — pizza, burgers, or desserts?"]],
])
...
def load_faiss_index():
    ...
    try:
        ...
        with open(FAISS_META_PATH, 'r', encoding='utf-8') as f:
            meta = json.load(f)
        # create model for embedding queries
...
faiss_index, faiss_meta, embed_model = load_faiss_index()
...
def api_chat():
    ...
    # If regex-based chatbot didn't find a response, try semantic search fallback
    if not response:
        # try FAISS semantic match if available
        if faiss_index is not None and faiss_meta is not None and embed_model is not None and message.strip():
            try:
                q_emb = embed_model.encode([message], convert_to_numpy=True)
                q_emb = q_emb.astype('float32')
                faiss.normalize_L2(q_emb)
                D, I = faiss_index.search(q_emb, 1)
                if I.shape[1] > 0 and I[0][0] >= 0:
                    meta = faiss_meta[I[0][0]]
                    response = meta.get('answer') or meta.get('question')
            except Exception:
                response = None
...
def api_search():
    """Return top-k similar QnA entries using FAISS."""
    if faiss_index is None or faiss_meta is None or embed_model is None:
        return jsonify({'error': 'FAISS index not found. Run preprocess.py to build the index.'}), 400
    ...
    try:
        ...
        for score, idx in zip(D[0], I[0]):
            if idx < 0 or idx >= len(faiss_meta):
                continue
            item = faiss_meta[idx]
            results.append({'question': item.get('question'), 'answer': item.get('answer'), 'score': float(score)})
