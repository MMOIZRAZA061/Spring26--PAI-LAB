...
# ---------- Attendance helpers ----------
def already_marked_recently(student_id, minutes=5):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    cutoff = (datetime.datetime.now(timezone.utc) - datetime.timedelta(minutes=minutes)).isoformat()
    c.execute("SELECT 1 FROM attendance WHERE student_id=? AND timestamp >= ? LIMIT 1", (student_id, cutoff))
...
def upload_face():
    ...
    for f in files:
        try:
            fname = f"{datetime.datetime.now(timezone.utc).timestamp():.6f}_{saved}.jpg"
...
def recognize_face():
    ...
    try:
        ...
        if emb is None:
            return jsonify({"recognized": False, "error":"No face detected or multiple faces in view. Please isolate one person."}), 200
        # attempt prediction
        ...
        if conf < 0.65:
            return jsonify({"recognized": False, "confidence": float(conf), "error":"Low confidence"}), 200
        ...
        if already_marked_recently(student_id, minutes=5):
            conn.close()
            return jsonify({
                "recognized": True,
...
# -------- Students page --------
@app.route("/students_page", methods=["GET"])
def students_page():
